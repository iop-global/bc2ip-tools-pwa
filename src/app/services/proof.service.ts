...
export class ProofService {
  ...
  async validate(entries: Entry[]): Promise<CryptoValidationResult> {
    ...
    const presentation = JSON.parse(
      await presentationEntry.getData(new TextWriter())
    ) as SignedPresentation;

    const nonCollapsedClaimFiles = Object.values(
      presentation.content.provenClaims[0].claim.content.files
    )
      .filter((f) => typeof f !== 'string')
      .map((f) => f as PresentationClaimFile);
    const integrityOK = await isIntegrityOK(nonCollapsedClaimFiles, entries);
    const signatureIsValid = isSignatureValid(
    ...
    const timestampFoundOnBlockchain = await lastValueFrom(
      this.sdkWebService.beforeProofExists(
        ProofService.extractProof(presentation)
      )
    );
  ...
  async getProofData(presentation: SignedPresentation): Promise<ProofData> {
    ...
    const txnStatus = await (
      await this.layer1Api()
    ).getTxnStatus(history.txid!);
    ...
    const statement =
      presentation.content.provenClaims[0].statements[0].content;
        ...
        return {
          ...
          owners:
            typeof file.owners === 'string'
              ? null
              : Object.values(file.owners).map(
                  (o) => (o as PresentationOwner).owner
                ),
          authors:
            typeof file.authors === 'string'
              ? null
              : Object.values(file.authors).map(
                  (o) => (o as PresentationAuthor).author
                ),
        };
      });
    ...
    return {
      ...
      sealedByProofCreator:
        typeof claim.sealer === 'string'
          ? null
          : new PublicKey(presentation.signature.publicKey).validateId(
              new KeyId(claim.sealer.keyId)
            ),
      projectId: claim.projectId,
      projectName:
        typeof claim.projectName === 'string' ? null : claim.projectName.value,
      projectDescription:
        typeof claim.projectDescription === 'string'
          ? null
          : claim.projectDescription.value,
      versionId: claim.versionId,
      versionDescription:
        typeof claim.versionDescription === 'string'
          ? null
          : claim.versionDescription.value,
