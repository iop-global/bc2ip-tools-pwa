...
childProcess.exec(`ng extract-i18n --format=json --output-path=${localeDir} --out-file=extracted-messages.json`, { cwd: '../../' }, (err, output) => {
    ...
    for (const lang of SUPPORTED_LANGS) {
        ...
        if (fs.existsSync(path)) {
            ...
            fs.writeFileSync(path, removeTrailingSpaces(JSON.stringify(currentTranslation, null, 2)) + '\n');
        }
        else {
            fs.writeFileSync(path, removeTrailingSpaces(JSON.stringify(extracted, null, 2)) + '\n');
        }
    }
